...
### 1. 정적 패터리는 이름을 가질 수 있어, 반활될 객체의 특성을 잘 표현할 수 있다.
...
추가하는 이러한 방식은 좋지 않다. 그래서 이를 정적 팩토리 메소드로 해결할 수 있는 것이다.
...
인터페이스를 정적펙터리 메소드의 반환 타입으로 지정하면서 구현체를 노출시키지 않을 수 있고 결과적으로 프로그래머가 알아야 하는 개수와 난이도가 낮아지는 것이다.
